...
class Model:
    ...
    def adjust_weights_and_bias(self, a):
        m, n = self.X.shape
        dw, db = self.find_adjustments()
        for i in range(n):
    ...
    def run_gradient_descent(self, a, e, m):
        cost_tracker = []
        for i in range(m):
            curr_cost = self.regression_type.compute_cost(self.regression_type.predict_with_current_params(), self.Y)
            cost_tracker.append(curr_cost)
            self.adjust_weights_and_bias(a)
